...
const TabList = ({ experiences }: Props) => {
  ...
  const sliderStyle =
    windowWidth <= sm
      ...
      : {
          top: `calc(${activeExperience}*5.5rem)`,
        };

  return (
    <div className="flex flex-col sm:flex-row text-sm md:text-base gap-10 md:gap-10 min-h-[300px]">
      {/* Sidebar */}
      <div className="font-mono text-xs sm:text-sm relative flex justify-start sm:flex-col overflow-scroll sm:overflow-auto sm:min-w-[180px]">
        {experiences.map(({ company }, i) => (
          <button
            key={getId()}
            className={`h-20 min-w-[120px] sm:w-auto sm:px-5 sm:!text-left capitalize hover:bg-accent-light hover:text-accent focus:outline-none focus:bg-accent-light focus:text-accent ${
